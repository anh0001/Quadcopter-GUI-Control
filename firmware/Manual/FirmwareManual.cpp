...
void loop() {
  ...
  for(uint8_t channel = 1; channel <= CHANNEL_AMOUNT; channel++) {
    PPMVal[channel-1] = ppm.latestValidChannelValue(channel, 1500);
    if(PPMVal[channel-1] > 1450 && PPMVal[channel-1] < 1550) {
      PPMVal[channel-1] = 1500;
    }
  }
  ...
  // Yaw
  ESCVal_1 += map(PPMVal[3], 1000, 2000, -16, 16);
  ESCVal_2 -= map(PPMVal[3], 1000, 2000, -16, 16);
  ESCVal_3 -= map(PPMVal[3], 1000, 2000, -16, 16);
  ESCVal_4 += map(PPMVal[3], 1000, 2000, -16, 16);
